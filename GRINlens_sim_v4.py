...
if mode==1:
    image_path = R"c:\Users\mrtmk\OneDrive\Desktop\Research\GRINlens\actual_machine\MTF_measure\testtest.tiff" # ← ここに読み込むPNGファイルのパスを指定
...
OPL = OPL_num / OPL_den +np.sqrt(d1**2+(FX2*wavelen*d2ideal)**2+(FY2*wavelen*d2ideal)**2)                  # shape = (Ny, Nx)
# z = np.arange(0, L + deltaz, deltaz)
...
print("OPL:axis_opl",OPL[NT//2,NT//2],d1+n1*L)
...
#mask = (R*wavelen_mm*zideal/d > rho0) & (R*wavelen_mm*zideal/d  <= 1)
rho0=0.99
k_A=8
A = np.sqrt(np.exp(-(np.sqrt((FX2*wavelen*d2ideal)**2+(FY2*wavelen*d2ideal)**2)*wavelen*d2ideal/R/rho0)**k_A))
...
plt.plot(X0[center_row,:], T3[center_row,:], label='Diffraction-limited',color='blue')
plt.xlim(0, X0[center_row,-1])
plt.xlabel('X0 [mm]')
plt.ylabel('OPL [mm]')
...
plt.figure(figsize=(7,4))
plt.plot(X2[NT//2,:], PSF[NT//2,:]/np.max(PSF[NT//2,:]))
